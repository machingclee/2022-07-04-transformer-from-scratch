...
def get_attn_pad_mask(seq_q, seq_k, pad=0):
    ...
    pad_attn_mask = seq_k.data.eq(pad).unsqueeze(
        1)  # [batch_size, 1, len_k], True is masked
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs):
        ...
        dec_outputs = self.pos_emb(dec_outputs.transpose(0, 1)).transpose(
            0, 1).to(device)  # [batch_size, tgt_len, d_model]
        dec_self_attn_pad_mask = get_attn_pad_mask(
            dec_inputs, dec_inputs).to(device)  # [batch_size, tgt_len, tgt_len]
        dec_self_attn_subsequence_mask = get_attn_subsequence_mask(
            dec_inputs).to(device)  # [batch_size, tgt_len, tgt_len]
